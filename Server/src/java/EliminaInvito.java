 ...
 * @author Davide
 */
public class EliminaInvito extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
      ...
      JSONObject obj=new JSONObject();
        try{
            String query3 = "DELETE FROM INVITO WHERE IDFESTA=? AND IDUTENTE=?";
            PreparedStatement st3 = DBManager.db.prepareStatement(query3);
            st3.setInt(1, idfesta);
            st3.setInt(2, idutente);
            st3.executeUpdate();
            st3.close();
obj.put("invitoeliminato", "true");
        } catch (SQLException e) {
obj.put("invitoeliminato", "false");
        }
        out.print(obj);
    }
