 ...
 * @author Davide
 */
public class InvitaFesta extends HttpServlet {
    ...
    idfesta=Integer.parseInt(request.getParameter("idfesta"));
     ...
     try{
            String query = "INSERT INTO INVITO (IDUTENTE,IDFESTA,DATAINVITO) VALUES(?,?,CURRENT_TIMESTAMP)";
            PreparedStatement st = DBManager.db.prepareStatement(query);
            st.setInt(1, idutente);
            st.setInt(2, idfesta);
            st.executeUpdate();
            obj.put("invitomandato", "true");
            }catch(Exception e){
                e.printStackTrace();
                obj.put("invitomandato", "false");
            }
      out.print(obj);
